...
public class Main extends Application {
    ...
    public void start(Stage stage) {
        ...
        BorderPane findMediaPane = new BorderPane();
        Scene findMediaPage = new Scene(findMediaPane,screenSize.getWidth(),screenSize.getHeight());
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Media Pages ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        Button mediaB6 = new Button("Find a Media");

        VBox mediaMenu= new VBox();
        mediaMenu.setSpacing(25);
        mediaMenu.setPadding(new Insets(30,30,30,30));
        mediaMenu.getChildren().addAll(mediaB1,mediaB2,mediaB3,mediaB4,mediaB5,mediaB6);
        ...
        mediaB6.setOnAction(e->stage.setScene(findMediaPage));
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Find Media Page ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        findMediaPane.setPadding(new Insets(50,50,50,50));
        findMediaPage.getStylesheets().add("mainPageStyle.css");



        TextField mediaTitle_findPage = new TextField();
        mediaTitle_findPage.setPromptText("Enter Media Title");
        mediaTitle_findPage.setAlignment(Pos.CENTER);

        TextField rating_findPage = new TextField();
        rating_findPage.setPromptText("Enter Movie Rating");
        rating_findPage.setAlignment(Pos.CENTER);

        TextField artist_findPage = new TextField();
        artist_findPage.setPromptText("Enter Artist Name");
        artist_findPage.setAlignment(Pos.CENTER);

        TextField songs_findPage = new TextField();
        songs_findPage.setPromptText("Enter Songs (seperated by (,) )");
        songs_findPage.setAlignment(Pos.CENTER);

        TextArea out1_findPage = new TextArea();
        out1_findPage.setEditable(false);
        out1_findPage.setMaxWidth(600);
        out1_findPage.setPrefHeight(400);
        out1_findPage.setStyle("-fx-font-size: 16;-fx-font-family: 'Arial Rounded MT Bold'");

        HBox findInputs = new HBox();
        findInputs.getChildren().addAll(mediaTitle_findPage,rating_findPage,artist_findPage,songs_findPage);
        findInputs.setSpacing(15);
        findInputs.setAlignment(Pos.CENTER);

        VBox findPageElements = new VBox();
        findPageElements.getChildren().addAll(findInputs,out1_findPage);
        findPageElements.setSpacing(30);
        findPageElements.setAlignment(Pos.CENTER);

        findMediaPane.setCenter(findPageElements);
        BorderPane.setAlignment(findPageElements,Pos.CENTER);


        // back button

        Button back_bt_findPage = new Button("Back");
        back_bt_findPage.setMaxWidth(400);
        Image backImage_findPage = new Image("back.png");
        ImageView backImageView_findPage = new ImageView(backImage_findPage);
        backImageView_findPage.setFitWidth(30);
        backImageView_findPage.setFitHeight(30);
        back_bt_findPage.setGraphic(backImageView_findPage);
        back_bt_findPage.setGraphicTextGap(15);

        findMediaPane.setBottom(back_bt_findPage);
        BorderPane.setAlignment(back_bt_findPage,Pos.CENTER_LEFT);


        ArrayList<Media> initialRes = mediaRental.searchMedia("","","","");
        if (initialRes.size() >0){
            out1_findPage.setStyle("-fx-font-size: 16;-fx-font-family: 'Arial Rounded MT Bold'");
            StringBuilder result = new StringBuilder();
            for (Media media : initialRes){
                result.append(media).append("\n\n");
            }
            out1_findPage.setText(result.toString());
        }else {
            out1_findPage.setText("No Result");
            out1_findPage.setStyle("-fx-alignment: Center ; -fx-font-size: 100;-fx-font-family: 'Arial Rounded MT Bold';-fx-text-fill: #cc0033");
        }


        //actions for buttons

        back_bt_findPage.setOnAction(e->{
            stage.setScene(mediaPage);
            out1_findPage.clear();
            mediaTitle_findPage.clear();
            rating_findPage.clear();
            artist_findPage.clear();
            songs_findPage.clear();
            out1_findPage.setStyle("-fx-font-size: 16;-fx-font-family: 'Arial Rounded MT Bold'");

        });

        mediaTitle_findPage.setOnKeyReleased(event -> {
            String title = mediaTitle_findPage.getText().trim();
            String rating = rating_findPage.getText().trim();
            String artist = artist_findPage.getText().trim();
            String songs = songs_findPage.getText().trim();

            ArrayList<Media> res = mediaRental.searchMedia(title,rating,artist,songs);
        ...
        rating_findPage.setOnKeyReleased(event -> {
        ...
        artist_findPage.setOnKeyReleased(event -> {
        ...
        songs_findPage.setOnKeyReleased(event -> {
