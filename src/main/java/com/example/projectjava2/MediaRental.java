...
public class MediaRental implements MediaRentalInt {
    ...
    @Override
    public ArrayList<Media> searchMedia(String title, String rating, String artist, String songs) {   //searching media depend on the title or any properties of them (like filters) also none of them(will return everything)
        ArrayList<Media> result = new ArrayList<>();   //creating
        ...
        for (Media m : mediaList) {      //moving on every media
            ...
            if (m instanceof Album) {  // if it is an album
                ...
                if(songs.isEmpty()){
                    ...
                    for (String s : songList) {   //to see if the songs array list match the searching parameters
                        if (!((Album) m).getSongs().contains(s.trim())) {
                            songsMatch = false;
                            break;
                        } else {
                            songsMatch = true;
            ...
            if (titleMatch && artistMatch && songsMatch && ratingMatch) {
                result.add(m);    // if the media matches all searching parameters add it to result arraylist
            }
        }
